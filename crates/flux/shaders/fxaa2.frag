                         ...
                         vec2 tcPixel, bool horzSpan, out vec2 posEdgeAA)
{
...
#ifdef FXAA_DEBUG_EDGE_NUM_STEPS
  fragData = vec4(float(stepsN) / float(EndpointSearchIterations), 0.f,
  ...
  if (needEdgeAA)
    ...
    if (nearestEndpointIsN)
      {
      fragData = vec4(1.f - dstN / maxDistance, 0.f, 0.f, 1.f);
      }
    else
      {
      fragData = vec4(0.f, 0.f, 1.f - dstP / maxDistance, 1.f);
      }
    }
  else
    {
    fragData = vec4(1.f, 1.f, 0.f, 1.f);
    }
  return FXAA_ABORT_EDGE_AA;
  ...
  if (needEdgeAA)
    ...
    fragData = vec4(-2.f * dst * invNegSpanLength, 0.f, 0.f, 1.f);
    return FXAA_ABORT_EDGE_AA;
    }
...
void main()
{
  ...
  if (lumRange < lumThresh)
    {
    fragData = vec4(rgbC, centerSample.a); // original color
    return;
    }
  ...
  if (blendSub > 0.f)
    {
    fragData = vec4(vec3(blendSub / SubpixelBlendLimit), 1.f);
    }
  else
    {
    fragData = vec4(rgbC, 1.f);
    }
  return;
  ...
  fragData = horzSpan ? vec4(0.f, 0.f, 1.f, 1.f)
  ...
  fragData = vec4(mix(rgbEdgeAA, rgbSub, blendSub), centerSample.a);
}
